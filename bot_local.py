...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
             STUDENT_NAME="Rosinta Novita",
            BATCH_ACADEMY="Rhea Night Online",
            GITHUB_REPO_LINK="https://github.com/rosintanov/telebot#telebot"
        )
...
def send_summary(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        total_conversion = df_campaign['total_conversion'].sum().astype(
            'int64')
...
def send_plot(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        axes[0].bar(x=df_campaign['age'].unique(),
                    height=df_plot['spent'], color='red')
        axes[0].set_ylabel('Total Spent', fontsize=8)

        # second subplot: total approved conversion per age group
        axes[1].bar(x=df_campaign['age'].unique(),
                    height=df_plot['approved_conversion'], color='black')
        axes[1].set_ylabel('Total Approved Conversion', fontsize=8)

        # third subplot: average CPC per age group
        axes[2].bar(x=df_campaign['age'].unique(),
                    height=df_plot['cpc'], color='red')
